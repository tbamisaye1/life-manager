import { AlertCircle, Hourglass, CheckCircle2, CalendarOff, Clock } from 'lucide-react'
import { cn } from '../../lib/cn'
import { deadlineStatus, hasTimeComponent } from '../../lib/format'
...
const TONE_STYLES = {
  ...
  urgent: { cls: 'bg-orange-50 text-orange-600', Icon: Clock },
...
export function DeadlineBadge({ date, done = false, className }) {
  ...
  const timed = !done && hasTimeComponent(date)
  const { cls, Icon } = TONE_STYLES[tone] || TONE_STYLES.none
  const TimedIcon = timed && tone === 'urgent' ? Clock : Icon
  return (
    <span className={cn('inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-xs font-medium whitespace-nowrap', cls, className)}>
      <TimedIcon className="h-3.5 w-3.5" />
      {label}
    </span>
  )
}
