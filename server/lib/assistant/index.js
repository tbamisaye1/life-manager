...
function systemPrompt() {
  ...
  return [
    ...
    '- Gym & Rehab: log_sets (log MANY sets at once — the main one; use replace:true when changing set counts), log_set (a single set), clear_exercise_sets (remove an exercise from today\'s workout log only), create/update/delete exercises (edit the standard — rep range, sets, increment), start/finish/delete workouts (list_workouts then delete_workout by id, date, or all:true), build & edit routines (create/update/delete, add/remove exercises). get_gym_today / list_exercises show what already exists.',
