...
router.get('/', async (req, res) => {
  const rows = await db.prepare('SELECT * FROM reply_queue ORDER BY done ASC, (due_date IS NULL), due_date ASC').all()
...
router.post('/', async (req, res) => {
  ...
  await db.prepare(`INSERT INTO reply_queue (id,person,platform,context,due_date,done,created_at,updated_at)
  ...
  res.status(201).json(decodeBooleans(await db.prepare('SELECT * FROM reply_queue WHERE id = ?').get(id), BOOLS))
})
...
router.patch('/:id', async (req, res) => {
  ...
  res.json(decodeBooleans(await db.prepare('SELECT * FROM reply_queue WHERE id = ?').get(req.params.id), BOOLS))
})

router.delete('/:id', async (req, res) => {
  await db.prepare('DELETE FROM reply_queue WHERE id = ?').run(req.params.id)
  res.json({ ok: true })
})
