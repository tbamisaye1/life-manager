...
router.get('/:id', (req, res) => {
  const im = db.prepare('SELECT * FROM improvements WHERE id = ?').get(req.params.id)
  if (!im) return res.status(404).json(httpError('Not found', 'NOT_FOUND'))
  const actions = db.prepare('SELECT * FROM improvement_actions WHERE improvement_id = ? ORDER BY created_at')
...
router.post('/', (req, res) => {
  ...
  db.prepare(`INSERT INTO improvements (id,title,emoji,category,summary,body,why,progress,created_at,updated_at)
  ...
  res.status(201).json(db.prepare('SELECT * FROM improvements WHERE id = ?').get(id))
})

router.patch('/:id', (req, res) => {
  const upd = buildUpdate('improvements', req.params.id, req.body, ALLOWED)
  if (upd) db.prepare(upd.sql).run(upd.params)
  res.json(db.prepare('SELECT * FROM improvements WHERE id = ?').get(req.params.id))
})

router.delete('/:id', (req, res) => {
  db.prepare('DELETE FROM improvements WHERE id = ?').run(req.params.id)
  res.json({ ok: true })
})
...
router.post('/:id/actions', (req, res) => {
  ...
  const id = newId()
  db.prepare('INSERT INTO improvement_actions (id,improvement_id,text,done,created_at) VALUES (?,?,?,0,?)')
    .run(id, req.params.id, text.trim(), now())
  res.status(201).json(decodeBooleans(db.prepare('SELECT * FROM improvement_actions WHERE id = ?').get(id), ['done']))
})

router.patch('/:id/actions/:actionId', (req, res) => {
  const a = db.prepare('SELECT * FROM improvement_actions WHERE id = ?').get(req.params.actionId)
  ...
  db.prepare('UPDATE improvement_actions SET done = ?, text = ? WHERE id = ?').run(done, text, req.params.actionId)
  res.json(decodeBooleans(db.prepare('SELECT * FROM improvement_actions WHERE id = ?').get(req.params.actionId), ['done']))
})

router.delete('/:id/actions/:actionId', (req, res) => {
  db.prepare('DELETE FROM improvement_actions WHERE id = ?').run(req.params.actionId)
  res.json({ ok: true })
})
