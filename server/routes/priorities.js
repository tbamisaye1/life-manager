...
router.get('/', async (req, res) => {
  const rows = await db.prepare('SELECT * FROM priorities WHERE active = 1 ORDER BY sort_order, created_at').all()
...
router.post('/', async (req, res) => {
  ...
  const max = (await db.prepare('SELECT COALESCE(MAX(sort_order), -1) m FROM priorities').get()).m
  await db.prepare(`INSERT INTO priorities (id,title,cadence,emoji,sort_order,active,created_at,updated_at)
  ...
  res.status(201).json(decorate(await db.prepare('SELECT * FROM priorities WHERE id = ?').get(id)))
})

// Toggle "done for this period".
router.post('/:id/check', async (req, res) => {
  const row = await db.prepare('SELECT * FROM priorities WHERE id = ?').get(req.params.id)
  if (!row) return res.status(404).json({ error: { message: 'Not found', code: 'NOT_FOUND' } })
  const newVal = isDoneForPeriod(row) ? null : now()
  await db.prepare('UPDATE priorities SET last_done_at = ?, updated_at = ? WHERE id = ?').run(newVal, now(), req.params.id)
  res.json(decorate(await db.prepare('SELECT * FROM priorities WHERE id = ?').get(req.params.id)))
})
...
router.delete('/:id', async (req, res) => {
  await db.prepare('DELETE FROM priorities WHERE id = ?').run(req.params.id)
  res.json({ ok: true })
})
