...
router.get('/', async (req, res) => {
  ...
  const rows = await db.prepare(`SELECT * FROM emails ${where} ORDER BY pinned DESC, received_at DESC`).all()
...
router.patch('/:id', async (req, res) => {
  ...
  res.json(decodeBooleans(await db.prepare('SELECT * FROM emails WHERE id = ?').get(req.params.id), BOOLS))
})
