...
router.get('/', (req, res) => {
  ...
  add(db.prepare("SELECT id, title FROM tasks WHERE title LIKE ? LIMIT 6").all(like),
    (r) => ({ type: 'task', id: r.id, title: r.title, subtitle: 'Task', path: '/tasks' }))

  add(db.prepare("SELECT id, title, start FROM events WHERE title LIKE ? ORDER BY start DESC LIMIT 6").all(like),
    (r) => ({ type: 'event', id: r.id, title: r.title, subtitle: 'Event', path: '/calendar' }))

  add(db.prepare("SELECT id, name, short_code FROM projects WHERE name LIKE ? OR short_code LIKE ? LIMIT 6").all(like, like),
    (r) => ({ type: 'project', id: r.id, title: r.name, subtitle: 'Project', path: `/projects/${r.id}` }))

  add(db.prepare("SELECT id, title, icon FROM pages WHERE archived = 0 AND (title LIKE ? OR body LIKE ?) LIMIT 8").all(like, like),
    (r) => ({ type: 'page', id: r.id, title: r.title || 'Untitled', subtitle: 'Page', icon: r.icon, path: `/notes/${r.id}` }))

  add(db.prepare("SELECT id, title, summary FROM improvements WHERE title LIKE ? OR summary LIKE ? LIMIT 6").all(like, like),
    (r) => ({ type: 'improvement', id: r.id, title: r.title, subtitle: 'Improvement', path: `/improvements/${r.id}` }))

  add(db.prepare("SELECT id, title FROM priorities WHERE active = 1 AND title LIKE ? LIMIT 5").all(like),
    (r) => ({ type: 'priority', id: r.id, title: r.title, subtitle: 'Priority', path: '/priorities' }))

  add(db.prepare("SELECT id, title FROM bored_items WHERE title LIKE ? LIMIT 5").all(like),
