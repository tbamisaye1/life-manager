...
router.get('/', (req, res) => {
  res.json(db.prepare('SELECT * FROM favorites ORDER BY sort_order, created_at').all())
})
...
router.post('/', (req, res) => {
  ...
  const max = db.prepare('SELECT COALESCE(MAX(sort_order), -1) m FROM favorites').get().m
  const id = newId()
  db.prepare('INSERT INTO favorites (id,label,icon,path,sort_order,created_at) VALUES (?,?,?,?,?,?)')
    .run(id, label.trim(), req.body.icon || 'Star', path, max + 1, now())
  res.status(201).json(db.prepare('SELECT * FROM favorites WHERE id = ?').get(id))
})
...
router.get('/recents', (req, res) => {
  res.json(db.prepare('SELECT * FROM recents ORDER BY visited_at DESC LIMIT 8').all())
})
...
router.post('/recents', (req, res) => {
  ...
  db.prepare(`INSERT INTO recents (path,label,icon,visited_at) VALUES (@path,@label,@icon,@ts)
  ...
  db.prepare(`DELETE FROM recents WHERE path NOT IN (SELECT path FROM recents ORDER BY visited_at DESC LIMIT 8)`).run()
  res.json({ ok: true })
})
