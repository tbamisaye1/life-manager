...
router.get('/', (req, res) => {
  const rows = db.prepare('SELECT * FROM notes ORDER BY pinned DESC, updated_at DESC').all()
  res.json(mapRows(rows, BOOLS))
})

router.post('/', (req, res) => {
  const ts = now(); const id = newId()
  db.prepare(`INSERT INTO notes (id,title,body,pinned,color,created_at,updated_at)
  ...
  res.status(201).json(mapRows([db.prepare('SELECT * FROM notes WHERE id = ?').get(id)], BOOLS)[0])
})
...
router.patch('/:id', (req, res) => {
  ...
  res.json(mapRows([db.prepare('SELECT * FROM notes WHERE id = ?').get(req.params.id)], BOOLS)[0])
})
