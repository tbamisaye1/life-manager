...
const googleRedirectUri = (req) => `${baseUrl(req)}/api/integrations/google/callback`
const microsoftRedirectUri = (req) => `${baseUrl(req)}/api/integrations/microsoft/callback`

router.get('/status', asyncRoute(async (req, res) => {
  res.json({
    google: await googleI.status(),
    microsoft: await microsoftI.status(),
    notion: await notionI.status(),
  })
}))

// --- Google (multi-account) ---
router.get('/google/connect', (req, res) => {
  const url = googleI.getAuthUrl(googleRedirectUri(req))
...
router.get('/google/callback', asyncRoute(async (req, res) => {
  ...
  try {
    await googleI.handleCallback(req.query.code, googleRedirectUri(req))
...
// --- Microsoft / Outlook (multi-account, calendar-only) ---
router.get('/microsoft/connect', (req, res) => {
  const url = microsoftI.getAuthUrl(microsoftRedirectUri(req))
  if (!url) return res.status(400).json(httpError('Microsoft is not configured. Add MICROSOFT_CLIENT_ID/SECRET to .env', 'NOT_CONFIGURED'))
  res.redirect(url)
})

router.get('/microsoft/callback', asyncRoute(async (req, res) => {
  const settings = `${baseUrl(req)}/settings`
  if (!req.query.code) return res.redirect(`${settings}?error=microsoft`)
  try {
    await microsoftI.handleCallback(req.query.code, microsoftRedirectUri(req))
    await microsoftI.syncAll().catch(() => {})
    res.redirect(`${settings}?connected=microsoft`)
  } catch (err) {
    res.redirect(`${settings}?error=${encodeURIComponent(err.message || 'microsoft')}`)
  }
}))

router.get('/microsoft/accounts', asyncRoute(async (req, res) => {
  res.json(await microsoftI.listAccounts())
}))

router.get('/microsoft/calendars', asyncRoute(async (req, res) => {
  res.json(await microsoftI.listCalendars())
}))

router.post('/microsoft/calendars/:id/selected', asyncRoute(async (req, res) => {
  await microsoftI.setCalendarSelected(req.params.id, req.body.selected !== false)
  res.json({ ok: true })
}))

router.post('/microsoft/accounts/:email/default', asyncRoute(async (req, res) => {
  await microsoftI.setDefaultAccount(req.params.email)
  res.json({ ok: true })
}))

router.patch('/microsoft/accounts/:email', asyncRoute(async (req, res) => {
  if (req.body.label) await microsoftI.setLabel(req.params.email, req.body.label)
  res.json({ ok: true })
}))

router.post('/microsoft/events', asyncRoute(async (req, res) => {
  const b = req.body || {}
  if (!b.title?.trim() || !b.start) return res.status(400).json(httpError('title and start are required', 'VALIDATION'))
  const start = b.start
  const end = b.all_day ? (b.end || start) : normalizeEventEnd(start, b.end || start)
  let { email, calendar_id: calendarId } = b
  if (!email || !calendarId) {
    const t = await microsoftI.defaultTarget()
    if (!t) return res.status(400).json(httpError('No Microsoft calendar connected.', 'NOT_CONNECTED'))
    email = email || t.email
    calendarId = calendarId || t.calendarId
  }
  const recurring = b.frequency || (Array.isArray(b.weekdays) && b.weekdays.length) || b.until || b.count
  ...
  const event = await microsoftI.createEvent({
    email, calendarId, title: b.title.trim(), start, end,
    allDay: !!b.all_day, location: b.location, notes: b.notes, flagship: b.flagship,
  })
  res.status(201).json(event)
...
router.post('/microsoft/sync', asyncRoute(async (req, res) => {
  res.json(await microsoftI.syncAll())
}))

router.delete('/microsoft/accounts/:email', asyncRoute(async (req, res) => {
  await microsoftI.disconnect(req.params.email)
  res.json({ ok: true })
}))
